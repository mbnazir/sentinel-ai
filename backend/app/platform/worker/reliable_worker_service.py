...
from app.platform.worker.reliable_worker_factory import build_reliable_queued_job_runner
from app.platform.worker.worker_settings import WorkerSettings

logger = logging.getLogger(__name__)


class ReliableWorkerService:
    """Production-oriented worker loop.

    It enqueues due schedules and executes queued jobs through reliability primitives.
    """
    ...
    async def tick(self) -> dict[str, list[str]]:
        ...
        try:
            ...
            attempted = await build_reliable_queued_job_runner(session).run_queued_jobs(
                limit=self.settings.max_jobs_per_tick
            )
            logger.info(
                "reliable_worker_tick_completed",
